...
Example: ["How to reset iPhone password", "how to go live on youtube", "Fix slow WiFi connection on Windows 11"]`;
  ...
  const body = {
    ...
    systemInstruction: {
      parts: [
        {
          text: "You are a research assistant that discovers trending tech help topics. Use web search to find real, commonly-asked questions. Return ONLY a JSON array of strings.",
        },
      ],
    },
  };
...
async function runBatch1(
  ...
  for (const category of categories) {
    // Check stop
    const { data: freshSettings } = await db
      .from("nightly_builder_settings")
  ...
  let dedupedTopics = allTopics.filter((t) => {
    ...
    return !allExistingTitles.some(
      (existing) => existing === topicLower || existing.includes(topicLower) || topicLower.includes(existing),
    );
  });
...
Return ONLY a JSON array of the INDEX NUMBERS (0-based) of topics that are UNIQUE and should be KEPT. Example: [0, 2, 5, 7]`;
        ...
        const dedupResult = await callGeminiFlash(
          apiKey,
          MODEL_LITE,
          dedupPrompt,
...
Return ONLY a valid JSON array. Example: [{"name": "Smart Home", "description": "Help with smart home devices", "icon": "Wifi"}]`;
      ...
      const catResult = await callGeminiFlash(
        apiKey,
        MODEL_FLASH,
        catPrompt,
        ...
        if (jsonMatch) {
          ...
          for (const newCat of newCats.slice(0, 5)) {
            ...
            const existing = categories.find(
              (c) => c.name.toLowerCase() === newCat.name.toLowerCase() || c.slug === slug,
            );
            if (existing) continue;

            const { data: insertedCat } = await db
              .from("categories")
              .insert({
                name: newCat.name,
                slug,
...
async function generateOneFromQueue(runId: string, batch: number, runDate: string) {
  const db = serviceClient();
  const settings = await ensureSettings(db);
  ...
  try {
    ...
    if (agentData.skipped) {
      ...
      const { data: currentRun } = await db
        .from("nightly_builder_runs")
        .select("articles_generated, articles_published")
        .eq("id", runId)
        .single();
