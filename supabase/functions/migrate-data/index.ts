...
Deno.serve(async (req) => {
  ...
  try {
    ...
    addLog("📂 Fetching categories from old DB...");
    ...
    if (catErr) {
      addLog(`❌ Error fetching categories: ${catErr.message}`);
      ...
      if (catInsertErr) {
        ...
        addLog(`✅ Migrated ${oldCategories.length} categories`);
      }
    } else {
      addLog("No categories found in old DB");
    }

    // ── 2. Migrate articles (paginated - handle >1000) ────
    addLog("📰 Fetching articles from old DB...");
    let allArticles: any[] = [];
    let page = 0;
    const pageSize = 500;
    ...
    while (true) {
      ...
      if (artErr) {
        addLog(`❌ Error fetching articles page ${page}: ${artErr.message}`);
        break;
      }
    ...
    if (allArticles.length > 0) {
      ...
      for (let i = 0; i < allArticles.length; i += 100) {
        ...
        if (artInsertErr) {
          addLog(`❌ Error inserting articles batch ${i / 100 + 1}: ${artInsertErr.message}`);
        } else {
          addLog(`  ✅ Inserted batch ${i / 100 + 1} (${chunk.length} articles)`);
        }
      }
      addLog(`✅ Migrated ${allArticles.length} articles total`);
    } else {
      addLog("No articles found in old DB");
    }

    // ── 3. Migrate article_feedback ────────────────────────
    addLog("👍 Fetching article feedback...");
    const { data: oldFeedback, error: fbErr } = await oldClient
      .from("article_feedback")
      .select("*");

    if (fbErr) {
      addLog(`⚠️ article_feedback: ${fbErr.message}`);
    } else if (oldFeedback && oldFeedback.length > 0) {
      const { error: fbInsertErr } = await newClient
        .from("article_feedback")
        .upsert(oldFeedback, { onConflict: "id" });
      if (fbInsertErr) {
        addLog(`❌ Error inserting feedback: ${fbInsertErr.message}`);
      } else {
        addLog(`✅ Migrated ${oldFeedback.length} feedback entries`);
      }
    } else {
      addLog("No feedback found");
    }

    // ── 4. Migrate contact_submissions ─────────────────────
    addLog("📬 Fetching contact submissions...");
    const { data: oldContacts, error: contactErr } = await oldClient
      .from("contact_submissions")
      .select("*");

    if (contactErr) {
      addLog(`⚠️ contact_submissions: ${contactErr.message}`);
    } else if (oldContacts && oldContacts.length > 0) {
      const { error: cInsertErr } = await newClient
        .from("contact_submissions")
        .upsert(oldContacts, { onConflict: "id" });
      if (cInsertErr) {
        addLog(`❌ Error inserting contacts: ${cInsertErr.message}`);
      } else {
        addLog(`✅ Migrated ${oldContacts.length} contact submissions`);
      }
    } else {
      addLog("No contact submissions found");
    }

    // ── 5. Migrate email_subscribers ───────────────────────
    addLog("📧 Fetching email subscribers...");
    const { data: oldSubs, error: subErr } = await oldClient
      .from("email_subscribers")
      .select("*");

    if (subErr) {
      addLog(`⚠️ email_subscribers: ${subErr.message}`);
    } else if (oldSubs && oldSubs.length > 0) {
      const { error: sInsertErr } = await newClient
        .from("email_subscribers")
        .upsert(oldSubs, { onConflict: "email" });
      if (sInsertErr) {
        addLog(`❌ Error inserting subscribers: ${sInsertErr.message}`);
      } else {
        addLog(`✅ Migrated ${oldSubs.length} email subscribers`);
      }
    } else {
      addLog("No subscribers found");
    }

    // ── 6. Migrate agent_runs ──────────────────────────────
    addLog("🤖 Fetching agent runs...");
    const { data: oldRuns, error: runErr } = await oldClient
      .from("agent_runs")
      .select("*");

    if (runErr) {
      addLog(`⚠️ agent_runs: ${runErr.message}`);
    } else if (oldRuns && oldRuns.length > 0) {
      const { error: rInsertErr } = await newClient
        .from("agent_runs")
        .upsert(oldRuns, { onConflict: "id" });
      if (rInsertErr) {
        addLog(`❌ Error inserting agent runs: ${rInsertErr.message}`);
      } else {
        addLog(`✅ Migrated ${oldRuns.length} agent runs`);
      }
    } else {
      addLog("No agent runs found");
    }

    // ── 7. Migrate agent_logs ──────────────────────────────
    addLog("📋 Fetching agent logs...");
    const { data: oldLogs, error: logErr } = await oldClient
      .from("agent_logs")
      .select("*");

    if (logErr) {
      addLog(`⚠️ agent_logs: ${logErr.message}`);
      ...
      for (let i = 0; i < oldLogs.length; i += 100) {
        ...
        if (lInsertErr) {
          addLog(`❌ Error inserting agent logs batch: ${lInsertErr.message}`);
        }
      }
      addLog(`✅ Migrated ${oldLogs.length} agent logs`);
    } else {
      addLog("No agent logs found");
    }

    // ── 8. Sync category article counts ────────────────────
    addLog("🔄 Syncing category article counts...");
    ...
    if (cats) {
      ...
      addLog("✅ Category article counts synced");
    }
